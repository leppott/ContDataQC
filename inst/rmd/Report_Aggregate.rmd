...
```{r, results='asis', echo=FALSE, warning=FALSE}
  ...
  cat(paste("**Report Date:** ",ContData.env$myReportDate,"\n\n",sep=""))
```
...
```{r, results='asis', echo=FALSE, warning=FALSE}
  ...
  cat(paste("**Data Type:** ",strFile.DataType,"\n\n",sep=""))  # need to do better
   ...
   myParameters.ALL     <- myNames.DataFields[ContData.env$myNames.DataFields %in% names(data.import)==TRUE] 
   myParameters.Lab.ALL <- myNames.DataFields.Lab[ContData.env$myNames.DataFields %in% names(data.import)==TRUE]
   # Filter out Discrete
...
#     use apply function to count number of records not equal to the expected value
    ...
    #
    myTable.DateTime.N <- aggregate(ContData.env$myName.SiteID ~ ContData.env$myNameDateTime, data=data.import,FUN=length)
...
#       #
     ...
     # plot(myPlot.Y,type="l",main=fun.myData.SiteID,xlab=myLab.Date,ylab=myPlot.Ylab,col="gray", xaxt="n")
       plot(myPlot.Y,type="p",main=fun.myData.SiteID,xlab=ContData.env$myLab.Date,ylab=myPlot.Ylab,col="gray", xaxt="n",cex=0.5)
...
```{r, results='asis', echo=FALSE, warning=FALSE}
 # BOTH plots
    ...
    if (ContData.env$myName.AirTemp %in% myParameters & ContData.env$myName.WaterTemp %in% myParameters == TRUE){##IF.Temp.START
      ...
      plot(myPlot.Y,type="p",main=fun.myData.SiteID,xlab=ContData.env$myLab.Date,ylab=myPlot.Ylab,col="green", xaxt="n",cex=0.5)
      axis(1,at=myAT,labels=myLab,tick=TRUE)
      ...
      myDiscrete <- paste(myPrefix.Discrete,ContData.env$myName.WaterTemp,sep=".")
      if (myDiscrete %in% myParameters.ALL == TRUE){##IF.Discrete.START
        data.plot[,myDiscrete] <- as.numeric(data.plot[,myDiscrete]) # coerce "NA" to NA
        data.plot.points <- na.omit(data.plot[,c(ContData.env$myName.DateTime,myDiscrete)]) # remove NA
        myPoints.X <- as.numeric(rownames(data.plot.points))
    ...
    #
    # Water, Temp vs Level
    if (ContData.env$myName.WaterTemp %in% myParameters & ContData.env$myName.WaterLevel %in% myParameters == TRUE){##IF.Temp_Level.START
      ...
      par.orig <- par(no.readonly=TRUE) # save original par settings
        ...
        myPlot.Y2 <- data.plot[,ContData.env$myName.WaterLevel]
        myPlot.Y2lab <- ContData.env$myLab.WaterLevel
        #
        #plot(myPlot.Y,type="l",main=fun.myData.SiteID,xlab=myLab.Date,ylab=myPlot.Ylab,col="blue", xaxt="n")
        plot(myPlot.Y,type="p",main=fun.myData.SiteID,xlab=ContData.env$myLab.Date,ylab=myPlot.Ylab,col="blue", xaxt="n",cex=0.5)
        axis(1,at=myAT,labels=myLab,tick=TRUE)
        # Add discrete, Water.Temp (only if present)
        ...
        myDiscrete <- paste(ContData.env$myPrefix.Discrete,ContData.env$myName.WaterLevel,sep=".")
        if (myDiscrete %in% myParameters.ALL == TRUE){##IF.Discrete.START
          data.plot[,myDiscrete] <- as.numeric(data.plot[,myDiscrete]) # coerce "NA" to NA
          data.plot.points <- na.omit(data.plot[,c(ContData.env$myName.DateTime,myDiscrete)]) # remove NA
          myPoints.X <- as.numeric(rownames(data.plot.points))
