...
```{r, results='asis', echo=FALSE, warning=FALSE}
  ...
  myTimeDiff.all <- difftime(data.import[,ConData.env$myName.DateTime],data.import[,ContData.env$myName.DateTime],units="mins")
...
#       #
  ...
  #    print("days where not the expected number of records")
      ...
      plot(myPlot.Y,type="l",main=fun.myData.SiteID,xlab=ContData.env$myLab.Date,ylab=myPlot.Ylab,col="gray", xaxt="n")
...
```{r, results='asis', echo=FALSE, warning=FALSE}
 # BOTH plots
    ...
    if (ContData.env$myName.AirTemp %in% myParameters & ContData.env$myName.WaterTemp %in% myParameters == TRUE){##IF.Temp.START
      ...
      plot(myPlot.Y,type="l",main=fun.myData.SiteID,xlab=ContData.env$myLab.Date,ylab=myPlot.Ylab,col="green", xaxt="n")
      axis(1,at=myAT,labels=ContData.env$myLab,tick=TRUE)
      ...
      par(mar=c(0,0,0,0))
      plot.new()
      legend(x="center",lty=1,col=c("green","blue"),legend=c("air","water"))
    ...
    if (ContData.env$myName.WaterTemp %in% myParameters & ContData.env$myName.WaterLevel %in% myParameters == TRUE){##IF.Temp_Level.START
      ...
      par.orig <- par(no.readonly=TRUE) # save original par settings
        ...
        #
        plot(myPlot.Y,type="l",main=fun.myData.SiteID,xlab=ContData.env$myLab.Date,ylab=myPlot.Ylab,col="blue", xaxt="n")
        axis(1,at=myAT,labels=ContData.env$myLab,tick=TRUE)
        # Add discrete, Water.Temp (only if present)
        myDiscrete <- paste(ContData.env$myPrefix.Discrete,ContData.env$myName.WaterTemp,sep=".")
        if (myDiscrete %in% myParameters.ALL == TRUE){##IF.Discrete.START
