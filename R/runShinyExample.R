...
#'
#' @details The Shiny app based on the R package ContDataQC is included in
...
runShinyExample <- function(){##FUNCTION.START
  ...
  if (system.file("shiny-examples"
                  , "ContDataQC"
                  , package = "ContDataQC") == "") {
    stop("Could not find example directory. Try re-installing `ContDataQC`."
         , call. = FALSE)
  }

  shiny::runApp(system.file("shiny-examples"
                            , "ContDataQC"
                            , package = "ContDataQC")
                , display.mode = "normal")
  #
