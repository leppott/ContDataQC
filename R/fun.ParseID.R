...
#'
#' @description This function will parse an ID string into SiteID, ID_Depth
#' , and ID_DepthUnits.
#'
#' @details Designed for use with depth profile data where the depth and
...
#' @param fun.Delim ID string delimiter.  Default = NA
#' (Default can be modified in config file; ContData.env$myDelim_LakeID)
#'
#' @return Returns a csv file to specified finelname with the full site ID
...
fun.ParseID <- function(fun.myFileName.In
  ...
  for (i in fun.myFileName.In){##IF~i~START
    ...
    df_split <- data.frame(do.call("rbind"
                                   , strsplit(as.character(df_import[
                                                          , fun.ColName.ID])
                                              , myDelim.strsplit
                                              , fixed=TRUE)))
    ...
    pat_matches <- regexpr(pat_units
                           , df_split[, "NonSiteID"]
                           , perl = TRUE
                           , ignore.case = TRUE)
    df_split[, "DepthUnits"]  <- regmatches(df_split[, "NonSiteID"]
                                            , pat_matches)
    #
    for(i in seq_len(nrow(df_split))){##FOR~i~START
      if(identical(df_split[i, "DepthUnits"], character(0))==TRUE) {
      ##IF~identical~START
        df_split[i, "DepthNumber"] <- df_split[i, "NonSiteID"]
        df_split[i, "DepthUnits"]  <- NA
      } else {
        df_split[i, "DepthNumber"] <- sub(df_split[i, "DepthUnits"]
                                          , "", df_split[i, "NonSiteID"])
        # df_split$Erik <- lapply(df_split[,c("DepthUnits", "NonSiteID")]
        #                         , 2, FUN=sub(x[, 1], "", x[, 2]))
      }##IF~identical~END
    }##FOR~i~END
